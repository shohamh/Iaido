...
$temporaryDirectory = Join-Path ([IO.Path]::GetTempPath()) ("iaido-sign-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $env:IAIDO_SIGN_KEY = $resolvedKey
    $env:IAIDO_SIGN_INPUT = $canonicalPath
    $env:IAIDO_SIGN_OUTPUT = $signaturePath
    @'
import java.nio.file.*;
import java.security.*;
import java.security.spec.*;
import java.util.*;
var pem = Files.readString(Path.of(System.getenv("IAIDO_SIGN_KEY")));
...
signature.update(Files.readAllBytes(Path.of(System.getenv("IAIDO_SIGN_INPUT"))));
Files.writeString(Path.of(System.getenv("IAIDO_SIGN_OUTPUT")), Base64.getEncoder().encodeToString(signature.sign()));
/exit
'@ | jshell -q | Out-Null
