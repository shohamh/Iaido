...
Design Iaido' version of Nintype's signature mechanic: splitting a single word's letters across simultaneous partial gestures/taps from two hands (e.g. swiping "th" with one hand while swiping/tapping "ere" with the other, concurrently, to produce "there"), including mixed taps for doubled letters (e.g. swipe then double-tap for "I'll").
