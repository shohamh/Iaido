# Iaido Stage 2: Minimal IME Design

## Goal

Make Iaido installable as a minimal Android input method that renders a
...
`IaidoInputMethodService` owns the current input session. It resets the
...
pointing to `res/xml/method.xml`. The service label is Iaido.
...
- Manual emulator verification, when an SDK is available: enable Iaido in
