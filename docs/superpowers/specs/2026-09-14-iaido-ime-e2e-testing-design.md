# Iaido end-to-end IME testing design

## Goal

Create a repeatable emulator test system that uses Iaido as a real Android
...
word paths into Android tests. `IaidoInputMethodService` remains the real
...
Iaido internals or bypass `InputConnection`; all text mutations in E2E
...
2. Enable and select Iaido with the `ime` shell commands through
...
- Iaido → reference keyboard → Iaido while the editor is focused.
...
- `logcat -d` filtered for Iaido, InputMethodManager, Compose, and fatal
  runtime messages.
