# Iaido Implementation Roadmap

> **For agentic workers:** This is a stage index, not an executable plan. Each stage gets its own detailed, bite-sized task plan (following the `superpowers:writing-plans` format) written just before that stage starts. Only [Stage 1](2026-09-13-iaido-stage-1-core-engine-bootstrap.md) has a detailed plan so far — write the next stage's detailed plan once the prior stage's tests are green and committed.

**Goal:** Implement Iaido end to end, in stages that each produce a working, demoable increment rather than one big-bang build.
...
- **Package naming** (not decided by any ticket — a reasonable implementation default, confirm/rename early if it doesn't fit): base package `com.iaido`, `core-engine` under `com.iaido.core`, `app` under `com.iaido.app`.

---

## Stage 1: Core-engine bootstrap + minimal single-finger recognition

**Detailed plan:** [2026-09-13-iaido-stage-1-core-engine-bootstrap.md](2026-09-13-iaido-stage-1-core-engine-bootstrap.md)
