# Iaido Stage 12 Dev Distribution and Release Packaging Implementation Plan
...
- Create: `docs/superpowers/plans/2026-09-14-iaido-stage-12-packaging.md`
...
- `gradle.properties` supplies `iaidoVersion=0.1.0`; the manifest records the same version.
...
**Interfaces:**
- `tools/deploy_app.ps1 -DeviceSerial <serial>` installs the debug APK to the explicitly selected ADB target; `-Build` assembles it first; `-Logcat` streams Iaido logs afterward.
- The manifest uses the placeholder vector icon and the app version comes from `iaidoVersion`.
...
- Modify: `docs/superpowers/plans/2026-09-14-iaido-stages-8-12-infrastructure.md`
