...
fun validateAppArchive(
    ...
    archive.packageName != installedPackageName -> AppArchiveValidation.Invalid("APK package does not match NinjaKeys")
...
class AppUpdateClient(
    ...
    fun update(onDownloadStarted: () -> Unit = {}): AppUpdateResult {
        ...
        val result = runCatching {
            ...
            clearIncoming()
            AppUpdateResult.Failed(failure.message ?: "Could not download the latest NinjaKeys update")
        }
        clearIncoming()
        return result
    }
    ...
    private fun openConnection(url: URL): HttpURLConnection = connectionFactory(url).apply {
        ...
        setRequestProperty("User-Agent", "NinjaKeys-App-Updater")
    }
