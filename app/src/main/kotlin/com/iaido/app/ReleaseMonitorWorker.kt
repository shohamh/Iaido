...
class ReleaseMonitorWorker(
        ...
        state = when (workflowStatus) {
            ReleaseWorkflowStatus.RUNNING -> {
                ...
                if (state.isFastPolling(nowMs) && releasePublished) {
                    ...
                    state.startFastPolling(nowMs, baseline)
                }
            }
            ReleaseWorkflowStatus.IDLE -> state.stopFastPolling()
            ReleaseWorkflowStatus.UNKNOWN -> state
        }
        ...
        if (probe is ReleaseProbe.Available &&
            ...
            when (val update = AppUpdateClient(context, channel = channel).update()) {
                ...
                is AppUpdateResult.ReadyToInstall -> {
                    ...
                    state = state.copy(
                        lastStagedRelease = probe.identity,
                        stagedVersionName = update.versionName.ifBlank { probe.versionLabel },
                        lastNotifiedRelease = probe.identity.takeIf { published },
                    )
                }
    ...
    private companion object {
        ...
        fun encodeIdentity(identity: ReleaseIdentity?): String? = identity?.let {
            listOf(it.channel.name, it.tagName, it.releaseId, it.assetUpdatedAt).joinToString("\u001f")
        }

        fun decodeIdentity(value: String?): ReleaseIdentity? {
        ...
        const val RELEASE_MONITOR_PREFS = "release-monitor"
    }
}
